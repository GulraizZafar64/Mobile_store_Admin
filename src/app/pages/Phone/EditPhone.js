...
function EditPhone(props) {
  ...
  const [settingColorLoading,setSettingColorLoading]=useState(true)
...
useEffect(() => {
  ...
  colors.filter((item)=>{
    data.colors.map((sub)=>{
      if(sub==item.value){
        SetColorsArray(prev => [...prev, {label:item.label,value:item.value}]);
      }
    })
  })
  
  setSettingColorLoading(false)
}, [])
  ...
  const saveUser = (values, setStatus, setSubmitting) => {
    ...
    fetch(`https://mobile-store-km17iiqc8-gulraizzafar64.vercel.app/api/v1/product/${data._id}`, {
        method: 'PUT',
        body: myForm
      }).then(res => res.json()
        .then(data => {
...
const updateProductImagesChange = (e) => {
      ...
      <div className="form">
        ...
        <div className="card-body">
          ...
          <div className="form-group row">
            ...
            <div className="col-lg-9 col-xl-6">
            <Select
            defaultValue={{label:data.company,value:data.company}}
  ...
  />
          ...
          <div className="form-group row">
            ...
            <div className="col-lg-9 col-xl-6">
            {!settingColorLoading?    <Select
                defaultValue={colorsArray}
        options={colors}
        isMulti
...

{
  ...
  <div style={{display:"flex"}}>
              {data.images.map((image, index) => (
                <img height={200} width={200} style={{marginLeft:"10px",borderRadius:"10px"}} key={index} src={image.url} alt="Product Preview" />
              ))}
            </div>
}
